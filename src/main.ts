...
async function main(): Promise<void> {
  ...
  consoleLogSink.write({ level: 'info', source: 'active-body', message: 'Phase 2 actuator scene ready' });
  ...
  function frame(now: number): void {
    ...
    renderer.render();
    status.textContent = `Morphodyne Phase 2 · ${mode} · tick ${simulation.tick} · x ${root.x.toFixed(2)} · y ${root.y.toFixed(2)} · z ${root.z.toFixed(2)} · yaw ${yaw.toFixed(2)}`;
